...
namespace MarsRover.Core.Repositories
{
    public sealed class RoverController : IRoverController
    {
        ...
        {
            bool isValidCommand = CommandHelper.IsValidInitialRoverPositionCommand(plateau.LowerCoordinate, plateau.UpperCoordinate, command);
        ...
        {
            bool isValidCommand = CommandHelper.IsValidMovementCommand(movementCommands);

            if (isValidCommand)
            {
                foreach (var command in movementCommands)
                {
                    if (CommandHelper.IsMoveForwardCommand(command))
                    {
        ...
        {
            bool isValidMovementForward = MovementHelper.TryMoveForward(plateau.LowerCoordinate, plateau.UpperCoordinate, rover.Position, out Coordinate nextCoordinate);
